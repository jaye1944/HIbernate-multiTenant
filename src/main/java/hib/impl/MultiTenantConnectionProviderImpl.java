...
public class MultiTenantConnectionProviderImpl extends
	...
	protected ConnectionProvider getAnyConnectionProvider() {
		return new TenantConnectionProvider("Master_H", getConnectionForTenant("Master_H"));
	}

	@Override
	protected ConnectionProvider selectConnectionProvider(String tenantCode) {
		return new TenantConnectionProvider(tenantCode, getConnectionForTenant(tenantCode));
	}
	...
	private Connection getConnectionForTenant(String tenantDb) {
		...
		try {
			conn = DriverManager.getConnection(conUrlPrefix + tenantDb + "?" +"user="+username+"&password="+password);
